          ...
          return Container(
            ...
            child: Column(
              children: [
                ...
                SizedBox(height: ResponsiveUtil.isMobile(context) ? 16 : 32),
                if (!ResponsiveUtil.isMobile(context))
                  Row(
                    children: [
                      Expanded(
                        child: AnswerButton(answer: state.question.answers[0]),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: AnswerButton(answer: state.question.answers[1]),
                      ),
                    ],
                  ),
                if (!ResponsiveUtil.isMobile(context))
                ...
                if (!ResponsiveUtil.isMobile(context))
                  Row(
                    children: [
                      Expanded(
                        child: AnswerButton(answer: state.question.answers[2]),
                      ),
                      const SizedBox(width: 16),
                      Expanded(
                        child: AnswerButton(answer: state.question.answers[3]),
                      ),
                    ],
                  ),
                if (ResponsiveUtil.isMobile(context))
